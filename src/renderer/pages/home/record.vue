<template>
  <div class="record">
    <el-table :data="tableData" border stripe height="100%" style="width:100%">
      <el-table-column width="60">
        <template slot-scope="scope">
          <el-button
            ...
            @click="handleRemear(scope.$index)"
          >重测</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="size" label="量块名义尺寸（mm）"></el-table-column>
      <el-table-column label="研合性/外观">
        <el-table-column prop="upface" label="上工作面" width="80">
          <template slot-scope="scope">
            <el-select
              v-if="scope.row.upface"
              v-model="scope.row.upface"
              style="width:100%"
              @change="faceSelectChange(scope.row.upface, 'upface', scope.$index)"
            >
              <el-option v-for="item in faceSelectOptions" :key="item" :label="item" :value="item"></el-option>
            </el-select>
          </template>
        </el-table-column>
        <el-table-column prop="downface" label="下工作面" width="80">
          <template slot-scope="scope">
            <el-select
              v-if="scope.row.downface"
              v-model="scope.row.downface"
              style="width:100%"
              @change="faceSelectChange(scope.row.downface, 'downface', scope.$index)"
            >
              <el-option v-for="item in faceSelectOptions" :key="item" :label="item" :value="item"></el-option>
            </el-select>
          </template>
      ...
      <el-table-column prop="detaLength" label="长度变动量（μm）"></el-table-column>
      <el-table-column prop="offsetLength" label="中心长度偏差（μm）">
        <template slot-scope="scope">
          <span>{{scope.row.offsetLength>0?`+${scope.row.offsetLength}`:scope.row.offsetLength}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="level" label="级" width="50"></el-table-column>
      <el-table-column prop="grade" label="等" width="50"></el-table-column>
    </el-table>
  </div>
</template>
